...
st.set_page_config(page_title="CONNECT GROUP | Vendas", layout="wide", page_icon="🎯")
...
if arquivos_locais:
    ...
    with st.sidebar:
        ...
        st.success(f"Foco: {MES_ALVO}")

    # REGRA: NOVO/ADITIVO + NÃO CANCELADO + (Ativado Março OU Sem data mas INPUT em Março)
    mask = (
    ...
    st.markdown(f"<div class='header-premium'><h1>🚀 PAINEL DE VENDAS - CONNECT GROUP</h1><div>📅 Período: {MES_ALVO}</div></div>", unsafe_allow_html=True)
